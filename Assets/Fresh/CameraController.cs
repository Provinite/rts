...
public class CameraController : MonoBehaviour
{
  ...
  [Tooltip(
    "How far from the edge of the screen the mouse can be to initiate camera movement"
  )]
  ...
  private static ArrayByEnum<Vector2, CameraDirection> _unitVectors =
    new ArrayByEnum<Vector2, CameraDirection>(
  ...
  private void _handleLateralMovement()
  {
    ...
    gameObject.transform.position =
      gameObject.transform.position
      + new Vector3(newDirection.x, 0, newDirection.y) * Time.deltaTime * 40;
  }
    ...
    if (previousZoomLevel != _zoomLevel)
    {
      ...
      else
      {
        ...
        TargetCamera.transform.Rotate(
          new Vector3(direction * _getZoomRotation(level), 0, 0)
        );
      }
    }
  }
