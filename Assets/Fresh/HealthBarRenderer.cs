...
public class HealthBarRenderer : MonoBehaviour
{
  ...
  void Start()
  {
    Physics.IgnoreLayerCollision(
      Globals.CAMERA_FRUSTUM_COLLIDER_LAYER_INDEX,
      Globals.PROJECTILE_LAYER_INDEX
    );

    _camera = GetComponent<Camera>();
    var obj = new GameObject(
      "HealthbarCameraViewCollider",
      new System.Type[] { typeof(MeshCollider), typeof(OnTriggerDelegate) }
    );
  ...
  private void handleEnter(Collider other)
  {
    ...
    var movableUnit =
      other.attachedRigidbody.gameObject.GetComponent<MovableUnit>();
    if (movableUnit != null)
    {
  ...
  void LateUpdate()
  {
    if (_frustumMesh == null)
    {
      ...
      var topRight = _screenPosToFarClipPos(
      ...
      _frustumMesh = SelectionMeshFactory.GeneratePyramidMesh(corners);
